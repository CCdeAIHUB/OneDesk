        ...
        private fun cacheScheme(desktopId: String, gatewayScheme: JSONObject) {
            ...
            for (pageIndex in 0 until pages.length()) {
                ...
                for (cellIndex in 0 until cells.length()) {
                    ...
                    val componentId = cell.optString("componentId", "").trim()
                    if (componentId.isBlank() || componentId.equals("null", ignoreCase = true)) {
                        continue
                    }
                    val componentName = componentNames[componentId]?.takeIf { it.isNotBlank() && !it.equals("null", ignoreCase = true) } ?: componentId
                    tiles.put(tile(componentName, "solar:bolt-circle-bold-duotone", "sky"))
                }
