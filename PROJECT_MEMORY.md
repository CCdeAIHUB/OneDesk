...
- Android cache schema version `3` clears older trusted desktop records and `scheme:*` snapshots once so previous demo/incorrect connection cache cannot appear as real connected content.
- After Android connects, it periodically refreshes the assigned scheme through the native shell so a desktop-side application can appear without restarting or manually reloading the mobile frontend.
