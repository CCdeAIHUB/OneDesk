...
- The Windows WebView2 shell must never use WinForms `TransparencyKey` for whole-window transparency. A color-keyed layered window can make the WebView2 DirectComposition surface fully invisible even though navigation, DOM, and accessibility all succeed. The validated Windows route is a non-layered DWM glass frame extended through the client area, a transparent WebView2 background, and frontend-controlled semi-transparent/frosted `app-shell` styling.
- Windows frontend pixel regression can be verified by launching with `ONEDESK_CAPTURE_FRONTEND=<absolute png path>`. After WebView2 navigation and bridge injection complete, the shell captures the real Chromium surface through the DevTools protocol. Normal launches do not set this variable and therefore do not create screenshots.
...
- Scheme application is stored per mobile device. A desktop-global active scheme must never leak to a newly paired mobile device; a mobile without an exact assignment receives an empty scheme descriptor.
- Applying a scheme from the desktop must show an in-progress loading/progress state from submission until the device acknowledges, reports failure, or the offline assignment is queued. Repeated submission and closing the apply dialog are disabled while this operation is active.
...
- Desktop editor dirty-state checks use stable normalized snapshots. A successful save refreshes the baseline, so navigation does not ask to save again; choosing “不保存” restores the complete saved editor snapshot before leaving instead of retaining transient in-memory edits.
- Desktop form controls use Vue-rendered `UiSelect` listboxes rather than Chromium-native select popups. Light-mode inputs, color controls, text areas, and listboxes require a visible surface and border, with consistent focus, disabled, keyboard, and Escape behavior.
- Desktop general settings persist theme and language preferences. “跟随系统” reacts to operating-system theme changes while the app is running.
- Desktop startup and gateway-port settings are shell-backed rather than frontend-only placeholders. Windows persists them under `%LOCALAPPDATA%\OneDesk\desktop-settings.json`; startup registration uses the current-user Run key, and changing the port restarts the gateway with rollback on failure.
...
- Android derives an app-scoped stable device key from `ANDROID_ID` through SHA-256 and never uploads the raw system identifier. Desktop pairing indexes trust records by this key so uninstalling and reinstalling the app reuses the original desktop-assigned device ID, remark, permissions, and scheme assignment instead of creating a duplicate device.
...
- Desktop JSON state reads and atomic replacements are serialized per normalized file path. Android scheme snapshot download, media materialization, cache-index commit, and old-cache cleanup are one serialized transaction; every media download uses a unique temporary file and replace move so push and fallback refresh cannot corrupt each other.
...
- Android page transitions must keep the incoming page rendered beneath the outgoing page so no animation can expose the WebView default background. Visual component roots fill their grid cells without applying component outer margins; page spacing belongs to the grid and cell definitions.
